...
// Merges this table to another given a merge function
...
// Performs a 4 way fold using the provided function. These mapped values are then merged.
fun <T, U, V> Table<T>.tableWedgeReduce(function: (T, List<T>) -> U, acc: (U, U, U, U) -> V): Table<V> =
    this.map { it.biWedgeReduce(function) { a, b -> a to b } }
        .mergeTo(this.transpose().map { it.biWedgeReduce(function) { a, b -> a to b } }
            .transpose()) { (a, b), (c, d) -> acc.invoke(a, b, c, d) }
